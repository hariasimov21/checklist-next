...
type Attachment = {
    id: string;
    name: string;
    url: string;   // path interno en Supabase (p.ej. cards/<cardId>/<uuid>-file.pdf)
    mime: string;
    size: number;
};
...
export function AttachmentsBar({
    ...
    async function handleFilesSelected(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        try {
            ...
            const next = [...json.attachments, ...list];
            setList(next);
            onChange?.(next);
        } catch (err) {
            console.error(err);
    ...
    async function openPreview(att: Attachment) {
        try {
            ...
            if (!json?.signedUrl) {
                console.error("Respuesta sin signedUrl:", json);
                ...
                <div className="flex flex-wrap gap-2">
                    {list.map(att => (
                        <button
                            ...
                            title={att.name}
                        >
                            <span className="truncate max-w-[160px]">{iconFor(att.mime)} {att.name}</span>
...
function PreviewBody({ att, signedUrl }: { att: Attachment; signedUrl: string }) {
    ...
    // Microsoft Office y similares: Word, Excel, PowerPoint
    if (
    ...
    // Cualquier otro tipo (ZIP, TXT, binarios, etc.)
    return (
        <div className="p-4 text-center">
